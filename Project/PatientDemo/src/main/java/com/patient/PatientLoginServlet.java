		...
		String userType = request.getParameter("userType");
		 ...
		 boolean isTrue = false;
	        ...
	        if (isTrue) {
	            ...
	            if ("patient".equals(userType)) {
	                // Retrieve patient details and forward to patient account page
	       ...
	       // Fetch doctor details from the database
	             //   List<Doctor> doctorDetails = PatientDBUtil.getDoctor(username);
	                
	               // if (doctorDetails != null && !doctorDetails.isEmpty()) {
	                	
	                    // Assuming the first doctor in the list is the one we need
	                //    Doctor doctor = doctorDetails.get(0);
	                 //   String dName = doctor.getdName();  // Get doctor's name
	                    
	                    // Store dName in session
	                  //  session.setAttribute("dName", dName);
	                    
	                // Redirect or forward to the relevant page
